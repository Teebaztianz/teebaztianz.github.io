/*----------------------------------------------------------------------------*/
/* Sauna Lite Responsive WordPress Theme */
/*----------------------------------------------------------------------------*/

Theme Name      :   Sauna Lite
Theme URI       :   https://www.themescaliber.com/free/wp-sauna-spa-wordpress-theme
Version         :   1.2.3
Tested up to    :   WP 4.9.6
Author          :   ThemesCaliber
Author URI      :   https://www.themescaliber.com
license         :   GNU General Public License v3.0
License URI     :   http://www.gnu.org/licenses/gpl.html

/*----------------------------------------------------------------------------*/
/* About Author - Contact Details */
/*----------------------------------------------------------------------------*/

Email       	:   support@themescaliber.com

/*----------------------------------------------------------------------------*/
/* Features */
/*----------------------------------------------------------------------------*/

Manage Slider, services and footer from admin customizer theme setting section.


/*----------------------------------------------------------------------------*/
/* Theme Resources */
/*----------------------------------------------------------------------------*/

Theme is Built using the following resource bundles.

1 - All js that have been used are within folder /js of theme.
- jquery.nivo.slider.js, License: MIT, License Url: https://opensource.org/licenses/MIT
- Bootstrap.js => https://github.com/twbs/bootstrap/releases/download/v3.1.1/bootstrap-3.1.1-dist.zip
  Code and documentation copyright 2011-2016 Twitter, Inc. Code released under the MIT license. Docs released under Creative Commons.

2 - Open Sans font - https://www.google.com/fonts/specimen/Open+Sans
	PT Sans font - https://fonts.google.com/specimen/PT+Sans
	Roboto font - https://fonts.google.com/specimen/Roboto
	License: Distributed under the terms of the Apache License, version 2.0 http://www.apache.org/licenses/LICENSE-2.0.html

3 - Images used from Pixabay.
	Pixabay provides images under CC0 license (https://creativecommons.org/about/cc0)

	Header image, Copyright IreneLasus
	License: CC0 1.0 Universal (CC0 1.0)
	Source: https://pixabay.com/en/girl-hair-redhead-beauty-women-1378142/

	Bundled header image, Copyright Clker-Free-Vector-Images
	License: CC0 1.0 Universal (CC0 1.0)
	Source: https://pixabay.com/en/scissors-hair-cut-comb-beauty-43984/

	Bundled header image, Copyright OpenClipart-Vectors
	License: CC0 1.0 Universal (CC0 1.0)
	Source: https://pixabay.com/en/dryer-hair-hairdresser-1293118/

	Bundled header image, Copyright Alanyadk
	License: CC0 1.0 Universal (CC0 1.0)
	Source: https://pixabay.com/en/lipstick-make-up-piktogram-make-up-1397049/

4	CSS bootstrap.css
    -- Copyright 2011-2018 The Bootstrap Authors
    -- https://github.com/twbs/bootstrap/blob/master/LICENSE
    
    JS bootstrap.js
    -- Copyright 2011-2018 The Bootstrap Authors (https://github.com/twbs/bootstrap/graphs/contributors)
    -- https://github.com/twbs/bootstrap/blob/master/LICENSE

	CSS nivo-slider.css
	-- Copyright 2012, Dev7studios Free to use and abuse under the MIT license.
    -- http://www.opensource.org/licenses/mit-license.php

    Font-awesome.css
    --Font Awesome 4.3.0 by @davegandy - http://fontawesome.io - @fontawesome
 	License - http://fontawesome.io/license (Font: SIL OFL 1.1, CSS: MIT License)

5  Customizer Licence
	All code, unless otherwise noted, is licensed under the GNU GPL, version 2 or later.
	2016 © Justin Tadlock.

All the icons taken from genericons licensed under GPL License.
http://genericons.com/

/*----------------------------------------------------------------------------*/
/* Changelog */
/*----------------------------------------------------------------------------*/

Version 1.0
i) Intial version Release

Version 1.1
i) Fix style errors.

Version 1.1.1
i) Fix style errors.

Version 1.1.2
i) Fix errors related to enque and links.

Version 1.1.3
i) Translation and other errors fixed.

Version 1.1.4
i) Given used images link in readme.txt

Version 1.1.5
i) Bug fixes

Version 1.1.6
i) console error fixed

Version 1.1.7
i) Change the layout.
ii) Removed unused functions.
iii) Added the static section with pages.

Version 1.1.8
i)   Added translation tag.
ii)  Added rtl tag.
iii) Added Woocommerce.
iv)  Added hooks.
v)   Delete the changelog file and added in readme file.

Version 1.1.9
i)   Tested Upto WordPress Version 4.9.1.
ii)  Done Styling Changes.
iii) Remove Unwanted css code.

Version 1.2.0
i) Resolved the errors.
ii) Removed buddypress tag.

Version 1.2.1
i) Update fontawesome.
ii) Update woocommerce.
iii) Update font url code.
iv) Resolved styling errors.

Version 1.2.2
i)	 Update bootstrap.
ii)  Resolved the theme errors.
iii) Added post format files and done the changes in template-part folder.
iv)	 Update the translation file in language folder.
v)	 Removed the unused css.

Version 1.2.3
i)	 Added color and fonts pallete.
ii)  Resolved minor theme issues.

Sauna Lite version :
==========================================================
Sauna Lite version is compatible with GPL licensed.

For any help you can mail us at support[at]themescaliber.com